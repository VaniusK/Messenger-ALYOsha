...
- **Аргументы (JSON)**:
  ...
  - `members` : list[int64]? - список пользователей(без айди создателя, айди создателя достается из токена). Список может быть пустым
- **Возвращает**:
  - `chat`: Chat
- **Коды возврата**:
  - `201`: Успешно
  - `400`: Число пользователей слишком большое или айди создателя находится в списке участников
