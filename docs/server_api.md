...
Получить список учатников чата
- **Возвращает**:
...
Удалить пользователя из чата(могут только владелец и админы)
- **Возвращает**:
...
### PUT /chats/{chat_id}/members/{member_id} (protected)
Изменить роль у участника чата(может только владелец)
- **Аргументы (JSON)**:
