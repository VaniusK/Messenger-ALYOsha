...
- **Аргументы (JSON)**:
  ...
  - `reply_to_id`: int64?
  - `forward_info`: object?
  - `type`: string? (по умолчанию text, возможные значения: text, voice, round, sticker)
- **Возвращает**:
  - `message`: Message
- **Коды возврата**:
  - `201`: Успешно
...
## Attachments
Работа с вложениями реализована через S3-хранилище Yandex Cloud Storage или любое другое. Для того, чтобы все работало в .env нужно заменить заглушки на ключи, ссылки и название бакета которые выдал вам сервис. 

При загрузке сообщений с сервера у каждого из них есть поле attachments, в котором находятся вложения привязанные к конкретному сообщению со всей необходимой информацией.

### GET chats/attachments/presigned-url (protected)
Получить presigned url для загрузки файла для конкретного сообщения.

- **Аргументы (JSON)**:
  - `chat_id`: int64
  - `original_filename`: string
  - `upload_as_file`: string ("true" or "false")
  - `message_id`: int64
- **Возвращает**:
...
- **Коды возврата**:
  - `200`: Успешно
  - `403`: Ошибка доступа(пользователь пытается получить presigned url для загрузки файла не в свой чат)

### Загрузка файла в хранилище
PUT-запрос на presigned url, который вы получили на предыдущем шаге. Обязательно нужно прикрепить в заголовке Content-Type значение, которое вы получили на предыдущем шаге. В теле запроса передается сам файл в бинарном виде.

### POST /chats/attachments/ (protected)
...
- **Коды возврата**:
  - `201`: Успешно
  - `403`: Ошибка доступа(пользователь пытается создать вложение не в свой чат)

### Загрузка файла из хранилища
GET-запрос на presigned url, который вы получили на предыдущем шаге. В теле запроса получаете сам файл в бинарном виде.
